...
describe('EnforcementGates', () => {
  ...
  describe('evaluateSecrets', () => {
    it('should detect API keys', () => {
      const result = gates.evaluateSecrets('const apiKey = "test-key-placeholder-abc123456789012345678901"');
      expect(result).not.toBeNull();
      expect(result!.decision).toBe('block');
    ...
    it('should provide redacted output in metadata', () => {
      const result = gates.evaluateSecrets('api_key = "test-key-placeholder-verylongsecretthatshouldberedacted"');
      if (result) {
